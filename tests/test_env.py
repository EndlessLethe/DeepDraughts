...
LastEditTime: 2021-11-09 19:16:26
Description: 
'''
...
def test_rule_cannot_jump_over_twice1():
    ...
    gui.game.current_board.set_board([12, 17, 44], [5], blacks_isking=[True])
...
def test_rule_cannot_jump_over_twice2():
    gui = GUI()
    gui.game.current_board.init_empty_board()
    gui.game.current_board.set_board([26, 46, 42, 37], [39], blacks_isking=[True])
    gui.game.change_player()
    gui.run()
...
if __name__ == "__main__":
    test_rule_cannot_jump_over_twice1()
    test_rule_cannot_jump_over_twice2()
